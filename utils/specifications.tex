...
Nous avons test\'e plusieurs algorithmes pour v\'erifier le bon fonctionnement des kilobots.\\
Ces algorithmes sont les suivants : \\

\smallskip
Pour v\'erifier le bon fonctionnement des communications inter-kilobots, nous avons impl\'ement\'e un algorithme de synchronisation (type luciole/firefly).\\
...
\textbf{S :} & envoie un message $<$Horloge$>$ &  & \textbf{R :} & r\'eception d'un message $<$Horloge$>$                                                                                                                                                                             \\
           &                                   &  &            & \begin{tabular}[c]{@{}l@{}}Si mon\_Horloge \textgreater Horloge            \\     décrémentation de mon\_Horloge d'une demi-période\\ Sinon    \\     incrémentation de mon\_Horloge d'une demi-période\end{tabular} \\
\end{tabular}
\end{table}

\smallskip
Pour v\'erifier le bon fonctionnement des communications inter-kilobots avec d\'eplacement, nous avons impl\'ement\'e un algorithme d'orbite.\\
...
\textbf{R :}    &                          &  & \textbf{R :}   & r\'eception d'un message $<>$                                                                                                                                                                                                               \\
                &                          &  &                & \begin{tabular}[c]{@{}l@{}} Si distance \textgreater= distance\_max\\ tourner à droite\\ avancer tout droit\\ Sinon SI distance \textless= distance\_min\\      touner à gauche \\ avancer tout droit\\ Sinon\\ avancer tout droit\\\end{tabular}
\end{tabular}
\end{table}
...
\begin{center}
\includegraphics[scale=0.5]{images/flashII.png}
\end{center}
